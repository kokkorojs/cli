<div align="center">
    ...
    &nbsp;&nbsp;|&nbsp;&nbsp;
    <span>
        <a href="https://gitee.com/rumicchi/kokkoro" target="_blank">gitee</a>
    </span>
    <br />
...
[![kokkoro](https://img.shields.io/badge/kokkoro-v0.1.0-57B497?style=flat-square&labelColor=FAFAFA&logo=npm&logoColor=FCAA00)](https://www.npmjs.com/package/kokkoro)
...
原项目 [yumemi](https://github.com/rumicchi/yumemi_bot) 主要围绕 [公主连结☆Re:Dive](https://priconne-redive.jp/) 开发相关功能，使用 ts 重构后代码风格参考了 [abot](https://github.com/takayama-lily/abot) ，并将原有特性做了整合：

- 多群配置管理，所有插件参数针对不同群聊均支持自定义修改
- 插件指令定制，启用的插件都可以编写配置文件独立修改指令
- web 可扩展性，每个插件均支持扩展独立的 web 页面及路由服务
- 默认扫码登录，能有效避免因操作不当导致账号风控、掉线等问题
- 伪本地数据库，基于 [lowdb](https://github.com/typicode/lowdb) ，可快速实现 JSON 信息存储
- 友好的脚手架，无需手动修改配置文件，全由命令自动构建，能防止因修改文件导致占用或缓存而产生的异常
- 账号并发冷启，abot 启动项目是登录一个主号来执行后续相关操作，kokkoro 可自定义修改登录的账号（避免瞬间登录 **过多账号** ，可能会出现被封号的风险）

~~项目断断续续也维护了快两年，起初并不打算大改，自从 abot 发布后写着写着就变成了高山的形状，就索性直接抄过来魔改了~~

## 安装

你可以选择下列任意一种方式来初始化你的项目
